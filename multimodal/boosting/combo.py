...
class MuComboClassifier(BaseEnsemble, ClassifierMixin, UBoosting):
    ...
    def _init_var(self, n_views, y):
        ...
        n_yi_s = np.zeros(n_classes, dtype=np.int64)
    ...
    def _indicatrice(self, predicted_classes, y_i):
        ...
        indicate_ones = np.zeros((self.n_views_, n_samples, self.n_classes_), dtype=np.int64)
        indicatrice_one_yi = np.zeros((self.n_views_, n_samples, self.n_classes_), dtype=np.int64)
        ...
        delta = np.ones((self.n_views_, n_samples, self.n_classes_), dtype=np.int64)
    ...
    def fit(self, X, y, views_ind=None):
        ...
        if self.n_classes_ == 1:
            ...
            self.estimator_weights_beta_ = np.zeros((self.n_iterations_, n_views), dtype=np.float64)
        ...
        self.estimator_weights_beta_ = np.zeros((self.n_iterations_, n_views, self.n_classes_), dtype=np.float64)
